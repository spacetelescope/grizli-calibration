...
The data on each calibrator was also dithered across the detector (and in some cases beyond)
to provide a track on how (if at all) the extraction varied with detector position. Figure
\ref{fig:detector_pos} shows the detector position (as calculated by \texttt{astropy.wcs}) 
for the data on both calibrator stars. The position is also available in downloadable CSV
files for each source, alongside the rootname and proposal id number (from somewhere TBD?).

%fig%%%%-------------------------------------------------------%%%%%%
\begin{figure}[h!]
\makebox[\textwidth][c]{
    \mbox{\includegraphics[width=.8\textwidth]{images/detector_position_compare.png}
    }}
